  ...
  return (
    <AdminShell email={admin.email} role={admin.role}>
      <div>
        ...
        <section aria-label="Store metrics" className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            <article key={label} className="rounded-3xl border border-black/10 bg-white p-5 shadow-sm">
              <p className="text-sm text-black/50">{label}</p><p className="mt-3 text-2xl font-semibold">{value}</p><p className="mt-1 text-xs text-black/45">{note}</p>
            </article>
          ))}
        </section>
        <section className="mt-8 grid gap-6 xl:grid-cols-2">
          <article className="rounded-3xl border border-black/10 bg-white p-6 shadow-sm">
            <h3 className="text-lg font-semibold">Fulfillment</h3><p className="mt-1 text-sm text-black/50">Current order lifecycle</p>
            <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3">
              {Object.entries(metrics.fulfillment).map(([status, count]) => (
                <div key={status} className="rounded-2xl border border-black/8 p-4"><p className="text-xs capitalize text-black/45">{status}</p><p className="mt-2 text-xl font-semibold">{count}</p></div>
              ))}
            </div>
          </article>
          <article className="rounded-3xl border border-black/10 bg-white p-6 shadow-sm">
            <h3 className="text-lg font-semibold">Inventory health</h3><p className="mt-1 text-sm text-black/50">Active stock signals</p>
        ...
        <section className="mt-8 rounded-3xl border border-black/10 bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-black/50">Latest order and payment events recorded by the store.</p>
          {metrics.recentActivity.length === 0 ? (
            <div className="mt-6 rounded-2xl bg-neutral-50 p-5 text-sm text-black/55">No recent activity has been recorded yet.</div>
          ) : (
            <div className="mt-6 divide-y divide-black/5">
              {metrics.recentActivity.map((event) => (
                <div key={event.id} className="flex flex-col gap-1 py-4 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                  <div className="min-w-0"><p className="text-sm font-medium">{formatEventType(event.eventType)}</p><p className="mt-1 truncate text-xs text-black/45">Order {event.orderId} · {event.actorType}</p></div>
                  <time dateTime={event.createdAt} className="shrink-0 text-xs text-black/45">{new Intl.DateTimeFormat("en-NG", { dateStyle: "medium", timeStyle: "short" }).format(new Date(event.createdAt))}</time>
                </div>
              ))}
            </div>
          )}
