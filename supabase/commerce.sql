...
create index if not exists orders_guest_access_expires_at_idx
  on orders(guest_access_expires_at)
  where guest_access_expires_at is not null;

-- The eight-argument overload is used by the current checkout route.
-- The original six-argument function remains for backward compatibility.
create or replace function create_pending_order(
  p_items jsonb,
  p_customer_email text,
  p_customer_name text,
